	...
	<section style="padding-right: 10%; padding-left: 10%; margin: 0 auto;">
		<form enctype="multipart/form-data" method="post"
			...
			<input type="hidden" name="p" value="${cv.currentPage }" /> <input
				...
				type="hidden" name="mb_idx" value="${sessionScope.mvo.mb_idx }" />
					...
					<dd>
						<c:if test="${not empty nv.fileList }">
							<c:forEach var="fvo" items="${nv.fileList }" varStatus="vs">
									${fvo.originalName }
									<i style="font-size: 15pt; color: red; cursor: pointer;"
									class="axi axi-delete2"
									onclick="deleteFile(${fvo.up_idx }, ${vs.count })"></i>
								<input type="hidden" name="delFile" id="delFile${vs.count }"
									value="0">
			...
			<c:if test="${role eq 'ROLE_ADMIN' }">
				<div style="padding-top: 1%; float: right;">
					<input type="submit" value="목록" class="btn btn-dark btn-sm"
						style="margin-right: 2px;" formaction="/board/notice.do">
					<input type="submit" value="수정" class="btn btn-primary btn-sm"
						formaction="/board/noticeUpdateOk.do">
				</div>
			</c:if>
		</form>
	</section>

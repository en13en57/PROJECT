...
<script type="text/javascript">
	...
	function sendUpdateParam() {
		$.ajax({
			type : "POST", 
			url : "board/QnAUpdateOk.do", // 컨트롤러에서 대기중인 URL 주소
			data : $('#qnaView').serialize(),
			dataType : "text",

			success : function(idx) { // 비동기통신의 성공일경우 success콜백으
...
<body class="is-preload landing">
	...
	<section style="padding-right: 10%; padding-left: 10%; margin: 0 auto;">
		<form action="${pageContext.request.contextPath}/board/QnAUpdateOk.do"
			method="post" id="qnaView" onsubmit="return formCheck();">
			<%-- 페이지번호, 페이지 크기, 블록크기를 숨겨서 넘긴다.  --%>
			<sec:csrfInput />
			<input type="hidden" name="p" value="${cv.pageSize }" /> <input
